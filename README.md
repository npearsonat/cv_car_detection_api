...
The model uses transfer learning, starting with Faster R-CNN weights pre-trained on the COCO dataset. This pretrained model already understood general object detection patterns from training on 80 different object classes. The model was then fine-tuned on a custom street car dataset of approximately 350 images with CSV annotations converted to COCO format. During 1000 training iterations using an 80/20 train/validation split, the model adjusted its internal parameters to better recognize cars in street scenes. The resulting trained weights were saved to a .pth file, which contains roughly 44 million numerical parameters representing the model's learned knowledge. When deployed, the API loads these trained weights to make predictions on new images based on the patterns it learned during training.
